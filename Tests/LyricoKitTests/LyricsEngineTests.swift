...
struct ActiveIndexTests {
    ...
    func beforeFirst() { #expect(LyricsEngine.activeIndex(in: lines, at: 0.5) == nil) }

    @Test("position exactly at a timestamp selects that line")
    func exactMatch() { #expect(LyricsEngine.activeIndex(in: lines, at: 5.0) == 1) }
    ...
    func afterLast() { #expect(LyricsEngine.activeIndex(in: lines, at: 999.0) == 3) }

    @Test("empty lines returns nil")
    func emptyLines() { #expect(LyricsEngine.activeIndex(in: [], at: 5.0) == nil) }
}
...
struct LyricsEngineTests {
    ...
    func fetchSuccess() async {
        ...
        let snap = PlaybackSnapshot(
            artist: "Artist", title: "Song", duration: 120, position: 5, isPlaying: true)
        await engine.update(with: snap)
    ...
    func sameTrackNoRefetch() async {
        let source = StubLyricsSource(document: .stub)
        let engine = LyricsEngine(lyrics: source)
        let snap1 = PlaybackSnapshot(
            artist: "A", title: "B", duration: 60, position: 1, isPlaying: true)
        let snap2 = PlaybackSnapshot(
            artist: "A", title: "B", duration: 60, position: 6, isPlaying: true)
    ...
    func currentIndexFollowsPosition() async {
        ...
        let engine = LyricsEngine(lyrics: source)
        await engine.update(
            with: PlaybackSnapshot(artist: "A", title: "B", position: 0, isPlaying: true))
        #expect(engine.currentIndex == 0)
        await engine.update(
            with: PlaybackSnapshot(artist: "A", title: "B", position: 6, isPlaying: true))
        #expect(engine.currentIndex == 1)
        await engine.update(
            with: PlaybackSnapshot(artist: "A", title: "B", position: 12, isPlaying: true))
