...
struct SwinsianClientParseTests {
  ...
  func emptyResponse() {
    #expect(SwinsianClient.parseResponse("") == nil)
  }

  @Test("'stopped' response means no current track")
  func stoppedResponse() {
    #expect(SwinsianClient.parseResponse("stopped") == nil)
  }

  @Test("error-prefixed response yields nil")
  func errorResponse() {
    #expect(SwinsianClient.parseResponse("error: bad track") == nil)
  }
  ...
  func trimsWhitespace() throws {
    let raw =
      ["id", "A", "B", "C", "100", "0", "playing"].joined(separator: separator) + "\n  "
    #expect(SwinsianClient.parseResponse(raw) != nil)
  }
}
