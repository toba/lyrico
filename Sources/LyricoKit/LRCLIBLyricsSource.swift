...
public struct LRCLIBLyricsSource: LyricsSource {
    ...
    public init(client: LRCLIBClient = .init()) { self.client = client }
    ...
    public func fetch(
        ...
        if let synced = track.syncedLyrics, !synced.isEmpty { return LRCParser.parse(synced) }
        if let plain = track.plainLyrics, !plain.isEmpty { return LRCParser.parse(plain) }
        return nil
    }
}
