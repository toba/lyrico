...
public enum LRCParser {
    public static func parse(_ source: String) -> LRCDocument {
        ...
        for rawLine in source.split(omittingEmptySubsequences: false, whereSeparator: \.isNewline) {
            ...
            if timestamps.isEmpty {
                if !text.isEmpty { rawLines.append(LyricLine(timestamp: nil, text: text)) }
            } else {
                for stamp in timestamps { rawLines.append(LyricLine(timestamp: stamp, text: text)) }
            }
        }

        let filtered = sawSyncedLine
            ? rawLines.filter { $0.timestamp != nil } : rawLines
        ...
        let adjusted:
            [LyricLine] = offset == 0
                ? filtered
                : filtered.map {
                    LyricLine(timestamp: $0.timestamp.map { $0 + offset }, text: $0.text)
                }
        ...
        return .init(lines: sorted, metadata: metadata)
    }
    ...
    private static func applyMetadata(
        key: String, value: String, into metadata: inout LRCMetadata
    ) {
        switch key {
            ...
            case "length":
                if let parsed = parseTimestamp(Substring(value)) { metadata.length = parsed }
            case "offset":
                if let milliseconds = Double(value) { metadata.offset = milliseconds / 1000.0 }
            default: break
        }
    }
}
